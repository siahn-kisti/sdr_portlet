...
public class CVModelPermission {
    ...
    public static void check(ServiceContext serviceContext, long targetId, int actionId)
        ...
        _log.debug("CV permission check : " + PermissionConstant.getStringforAction(actionId));
    ...
    public static boolean contains(ServiceContext serviceContext, long targetId, int actionId)
        ...
        switch (actionId) {
            ...
            case PermissionConstant.SDR_ACTION_DELETE :
                dts = DataTypeSchemaLocalServiceUtil.getDataTypeSchema(targetId);
                if (serviceContext.getUserId() == dts.getUserId()) return true;
                return false;
