		...
		<div class="listPopupArea">
			<liferay-ui:search-container emptyResultsMessage="Data-Not-Found" iteratorURL="${iteratorURL}">
				...
				<liferay-ui:search-container-results
						results="<%=DatasetServiceUtil.getList((int)searchContainer.getStart(), (int)searchContainer.getEnd(), keywords, (long)dataTypeId, WorkflowConstants.STATUS_APPROVED, serviceContext)%>"
...
<aui:script use="aui-base">
 	...
 	A.one('#<portlet:namespace/>datasetPopupFm').delegate('click',function(event){
 		...
 		var tempVal = '<%=request.getParameter("viewType")%>';
		var randomVal = '<%=request.getParameter("randomVal")%>';

		Util.getOpener().Liferay.fire('detail_datasetPopup_'+randomVal, result);
 		Util.getWindow().hide();
