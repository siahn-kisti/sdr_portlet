...
<%
    ...
    String ppId = ParamUtil.getString(request, "ppId");
    String curationIdList = ParamUtil.getString(request, "curationIdList");
%>
...
<aui:form method="post" name="compositionPopupFm" >
	<div class="input-append">
        ...
        <c:if test="${dataTypeName != null && dataTypeName != ''}">
            <aui:input type="hidden" inlineField="true" label="DataType" name="keyword" size="30"  style="margin-bottom:-12px;"/>
            <aui:input type="hidden" label="dataTypeId" name="dataTypeId" readonly="true" style="margin-bottom:-12px;"/><br />
            <aui:input type="text" label="DataType" name="dataTypeName" readonly="true" style="margin-bottom:-12px;"/>
        </c:if>
        <c:if test="${dataTypeName == null || dataTypeName == ''}">
...
<script>
	...
	YUI().use('aui-base','aui-form-validator', function(A) {
		...
		A.one('#<portlet:namespace/>compositionPopupFm').delegate('click',function(event){
            ...
            var ppId = "<%=ppId%>";
            var curationIdList = "<%=curationIdList%>";

            var ppIdValue = curationIdList.split(",");

            // detail ppid check
            if(ppId == result.value){
                alert("Can not be selected");
                return;
            }

            if(ppIdValue.length > 0){
                for(var i = 0; i < ppIdValue.length; i++){
                    if(result.value == ppIdValue[i]){
                        alert("Duplicate selection is not possible.");
                        return;
                    }
                }
            }

            Liferay.Util.getOpener().popup_compositionPopup(result);
			Liferay.Util.getWindow().hide();
		},'.selector-button');

		A.one('#<portlet:namespace/>compositionPopupFm').delegate('click',function(event){
			var f = document.<portlet:namespace/>compositionPopupFm;
            f.action = '${compositionPopupURL}';
            f.submit();
		},'.search-button');
