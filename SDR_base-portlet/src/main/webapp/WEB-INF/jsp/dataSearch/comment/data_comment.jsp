...
<%--id="commentsPanelContainer" persistState="${true}">--%>

<%--<liferay-ui:panel collapsible="${false}" extended="${true}"--%>
<%--id="ratingPanel" persistState="${true}"--%>
<%--title='<%=LanguageUtil.get(pageContext, "Rating")%>' cssClass="rating-panel">--%>
<%--<c:choose>--%>
<%--<c:when test="${!empty collection}">--%>
<%--<liferay-ui:ratings className="<%= Collection.class.getName() %>"--%>
<%--classPK="${collection.collectionId }" type="stars"/>--%>
...
<%--id="commentsPanel" persistState="${true}"--%>
