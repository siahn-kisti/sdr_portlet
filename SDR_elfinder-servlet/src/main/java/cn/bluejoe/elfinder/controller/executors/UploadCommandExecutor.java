...
//				return newFile;
				...
				if(file.getParent() != null) {
					newFile = new FsItemEx(dir, file.getParentFile().getPath() + "/" + p.getFileName().toString());
				} else {
		...
		{
			processUpload(request, uploads, fw);
		}
	...
	private void processUpload(HttpServletRequest request, MultipleUploadItems uploads, FileWriter fw)
		throws IOException
	{
		/* - 원본코드
		...
		String[] upload_path = request.getParameter("upload_path[]").toString().split(",");
		int num = 0;
		for (FileItemStream fis : uploads.items("upload[]"))
		{
			if(upload_path[num].indexOf(fis.getName()) != -1) {
				// TODO: "실제 파일 명"과 "upload_path[]"의 파일명이 일치하는지 확인 필요
				String name = "";

				if(upload_path[num].substring(0, 1).equals("/")) {
					name = upload_path[num].substring(1, upload_path[num].length());
				}
