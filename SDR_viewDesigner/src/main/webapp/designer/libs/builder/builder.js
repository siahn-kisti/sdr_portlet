		...
		fn = function(component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
				...
				} else if (property.htmlAttr)
				{
					...
					{
					    if(value.indexOf("medium")){
                            element = element.css(property.key, value.substring(0,value.indexOf("medium")) + "px");
                        }
...
Vvveb.Builder = {
	...
	loadControlGroups : function() {
		...
		for (group in Vvveb.ComponentsGroup)
		{
			...
			for (i in components)
			{
				...
				if (component)
				{
					item = $('<li data-section="' + group + '" data-type="' + componentType + '" data-search="' + component.name.toLowerCase() + '">' +
						'<a href="#" title="'+ component.name +'" >' + component.name + '</a>' + '</li>');
					
					//guide append
					if(component.guide){
						item.find('a').append('<i class="la la-question guide"> </i>');

						Vvveb.guide.add(component);
					}
					...
					if (component.image) {
						...
						item.attr('style','padding-top:10px;');
					}
	...
	selectNode:  function(node) {
        ...
        if(offset.left + target.outerWidth() < 137){
            jQuery("#select-actions").css({
                "right" : "-120px"
            });
        }else{
            jQuery("#select-actions").css({
	...
	_initHightlight: function() {
		...
		this.frameBody.on("mousemove touchmove", function(event) {
            ...
            if (event.target && isElement(event.target))
			{
				...
				if (self.isDragging)
				{
					...
					var drag_hr = $(self.iframe).contents().find('.drag_hr');
					drag_hr.remove();

					try {
						if (event.originalEvent)
						{
							drag_hr = $("<div class='drag_hr' style='width:100%;height:2px;background-color: royalblue;'></div>");
							if ((offset.top  < (event.originalEvent.y - halfHeight)) || (offset.left  < (event.originalEvent.x - halfWidth)))
							{
								if (isIE11)
									self.highlightEl.append(drag_hr);
								else
									drag_hr.appendTo(parent);
							} else
							{
								if (isIE11)
									self.highlightEl.prepend(drag_hr);
								else
									drag_hr.prependTo(parent);
							};
						}
		...
		this.frameBody.on("mouseup touchend", function(event) {
            if (self.isDragging)
			{
                ...
                if (event.target && isElement(event.target))
				{
                    ...
                    if(target.hasClass("gridrow-nocontent")){
                        target.removeClass("gridrow-nocontent");
                    }
		...
		this.frameBody.on("click", function(event) {
			if (event.target)
			{
				...
				//gridrow col
                if(target.hasClass("gridrow-col")){
                    node = target.parent().get(0);
                ...
                if(target.attr("contenteditable")) return false;
                
                //table th td
                if(target.prop("tagName")=="TH" || target.prop("tagName")=="TD") {
                    node = target.parent().parent().get(0);

                    if(jQuery(node).prop("tagName")=="TBODY" || jQuery(node).prop("tagName")=="THEAD")
                        node = target.parent().parent().parent().get(0);
		...
		$("#delete-box").on("click", function(event) {
			...
			node = self.selectedEl.get(0);
            nodeParent = jQuery(node).parent(0);
			...
			//해당 컴포넌트가 모두 삭제됬을시 기존 스크립트 제거
			var body = Vvveb.Builder.frameBody;
			var nodeClass = self.selectedEl.attr('class');
			if($("."+nodeClass, body).length<=0){
				$("."+nodeClass+"-script", body).remove();
			}

            //gridrow-col 이고 컴포넌트가 모두 삭제됬을시 gridrow-nocontent class 추가 
            if(nodeParent.hasClass("gridrow-col")){
                if(nodeParent.contents().length<=0)
                    nodeParent.addClass("gridrow-nocontent");
            }
	...
	_initDragdrop : function() {
		...
		$('#components ul > li > ol > li').on("mousedown touchstart", function(event) {
			...
			//------------------------------------------------------
			var isGuide = $target.hasClass('guide');
			if(isGuide){
				if(component.guide){
					Vvveb.guide.open(componentType);
				}
				return false;
			}
			//------------------------------------------------------
			...
			if (Vvveb.dragIcon == 'html')
				...
				if($this.css("background-image")!='none'){
					...
					self.iconDrag = $('<label>'+$this.data("search")+'</label>').attr({"id": "component-clone"}).
					css({'z-index':100, 'position':'absolute', 'width':'64px', 'height':'64px', 'top': event.originalEvent.y, 'left': event.originalEvent.x});
				}
...
Vvveb.Gui = {
	...
	download : function () {
		...
		if ('download' in link)
		{
            ...
            if(isIE11){
                var htmlText = Vvveb.Builder.getHtml();
                var blobObject = new Blob([htmlText]);
	...
	fullscreen : function () {
        launchFullScreen(document.body, document); // the whole page
	},
	...
	leftPanelToggle : function(){
		var l = $("#vvveb-builder").hasClass('hide-left-panel');
		var r = $("#vvveb-builder").hasClass('hide-right-panel');
		if(!l){
			$(this).removeClass("active");
			$("#vvveb-builder").addClass("hide-left-panel");
			if(r) $("#vvveb-builder").addClass("hide-panel");
		}else{
			$(this).addClass("active");
			$("#vvveb-builder").removeClass("hide-left-panel");
			$("#vvveb-builder").removeClass("hide-panel");
		}
	},
	...
	rightPanelToggle : function(){
		...
		if(!r){
			$(this).removeClass("active");
			$("#vvveb-builder").addClass("hide-right-panel");
			if(l) $("#vvveb-builder").addClass("hide-panel");
		}else{
			$(this).addClass("active");
			$("#vvveb-builder").removeClass("hide-right-panel");
			$("#vvveb-builder").removeClass("hide-panel");
		}
	},
...
Vvveb.FileManager = {
	...
	init: function() {
		...
		$("#filemanager").on("click", ".header", function (e) {
			$("#filemanager").toggleClass("hide");
			$("#filemanager .tree").toggleClass("hide");
		});
	},
...
// Toggle fullscreen
function launchFullScreen(elem, doc) {
    //ie
...
Vvveb.guide = {
	...
	open: function(componentType){

		$('#guide-modal').modal();
		

		if(componentType==undefined && this.active=='') componentType = 'Home/Home';
		this.selected(componentType);
	},
	...
	add: function(component) {
		...
		var guideListLi = $('#guide-modal #guide-list ul');
		var item = $('<li data-component-type="'+component.type+'"><a>'+component.type+'</a></li>');
		item.click(function(){
			var id = $(this).data('componentType');
			self.selected(id);
		});
		
		if(component=='Home/Home') guideListLi.prepend(item);
		else guideListLi.append(item);
	},
	
	selected: function(componentType){
		if(componentType==undefined) return;
		var component = this.components[componentType];
		if(component==undefined) return;
		if(component.guide==undefined) return;
		
		this.active = componentType;
		$('#guide-modal #guide-list ul li').each(function(){
			if($(this).data('componentType')==componentType){
		...
		var html = '<h3>'+componentType+'</h3> <hr/>';
		html += component.guide;
		
		$('#guide-modal #guide-html').html(html);
	},

	empty: function(){
		$('#guide-modal #guide-list ul').empty();
		for(var i in this.components){
			var component = this.components[i];
			if(component['type']=='Home/Home'){
				this.add(component);
			}else{
				delete component;
			}
