...
public class TasksPortlet extends MVCPortlet {
	...
	public void addTask(ActionRequest request, ActionResponse response) throws Exception {
		...
		_log.debug("### TasksPortlet addTask");
	...
	private Object _updateTask(ActionRequest request) throws PortalException, SystemException {
		...
		ServiceContext serviceContext = ServiceContextFactory.getInstance(Object.class.getName(), request);

		Object task = null;

		if (taskId <= 0) {
			// task = TaskLocalServiceUtil.addTask(name, description, status, serviceContext);
			...
			// task = TaskLocalServiceUtil.updateTask(taskId, name, description, status, serviceContext);
		}
